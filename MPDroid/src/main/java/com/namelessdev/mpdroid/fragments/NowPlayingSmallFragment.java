...
public class NowPlayingSmallFragment extends Fragment implements StatusChangeListener,
    ...
    public View onCreateView(final LayoutInflater inflater, final ViewGroup container,
        ...
        // Make the song info area (cover + title/artist) clickable to expand the full Now Playing screen
        // The RelativeLayout containing cover and song info is the first child of the root LinearLayout
        if (view instanceof ViewGroup) {
            ViewGroup rootLayout = (ViewGroup) view;
            if (rootLayout.getChildCount() > 0) {
                View songInfoArea = rootLayout.getChildAt(0);
                songInfoArea.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        // Expand the sliding panel to show full Now Playing screen
                        Activity activity = getActivity();
                        if (activity instanceof MainMenuActivity) {
                            ((MainMenuActivity) activity).expandNowPlaying();
                        }
                    }
                });
                songInfoArea.setClickable(true);
            }
        }
