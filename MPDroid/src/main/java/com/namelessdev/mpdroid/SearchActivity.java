...
public class SearchActivity extends MPDroidActivity implements OnMenuItemClickListener,
    ...
    private ViewGroup mListAlbumsFrame = null;

    private ListView mListArtists = null;

    private ViewGroup mListArtistsFrame = null;

    private ListView mListSongs = null;

    private ViewGroup mListSongsFrame = null;
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        // Ensure the content view respects window insets for system bars
        ...
        // Initialize list view frames and list views
        final LayoutInflater inflater = LayoutInflater.from(this);
        
        // Initialize Artists list view frame (browse.xml has SwipeRefreshLayout > FrameLayout with ListView)
        View artistsRoot = inflater.inflate(R.layout.browse, null, false);
        // Find the ListView first, then get its parent FrameLayout
        ListView tempArtistsList = artistsRoot.findViewById(android.R.id.list);
        if (tempArtistsList != null && tempArtistsList.getParent() instanceof ViewGroup) {
            mListArtistsFrame = (ViewGroup) tempArtistsList.getParent();
            // Remove from SwipeRefreshLayout parent so we can add it to ViewPager
            ((ViewGroup) mListArtistsFrame.getParent()).removeView(mListArtistsFrame);
            // Ensure proper layout parameters for ViewPager
            mListArtistsFrame.setLayoutParams(new ViewGroup.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        } else {
            // Fallback: create a simple FrameLayout with ListView
            mListArtistsFrame = new FrameLayout(this);
            mListArtistsFrame.setLayoutParams(new ViewGroup.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
            tempArtistsList = new ListView(this);
            tempArtistsList.setId(android.R.id.list);
            mListArtistsFrame.addView(tempArtistsList);
        }
        mListArtists = tempArtistsList;
        mNoResultArtistsView = mListArtistsFrame.findViewById(R.id.noResultLayout);
        View loadingArtists = mListArtistsFrame.findViewById(R.id.loadingLayout);
        if (loadingArtists != null) {
            loadingArtists.setVisibility(View.GONE);
        }
        
        // Initialize Albums list view frame
        View albumsRoot = inflater.inflate(R.layout.browse, null, false);
        ListView tempAlbumsList = albumsRoot.findViewById(android.R.id.list);
        if (tempAlbumsList != null && tempAlbumsList.getParent() instanceof ViewGroup) {
            mListAlbumsFrame = (ViewGroup) tempAlbumsList.getParent();
            // Remove from SwipeRefreshLayout parent so we can add it to ViewPager
            ((ViewGroup) mListAlbumsFrame.getParent()).removeView(mListAlbumsFrame);
            // Ensure proper layout parameters for ViewPager
            mListAlbumsFrame.setLayoutParams(new ViewGroup.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        } else {
            mListAlbumsFrame = new FrameLayout(this);
            mListAlbumsFrame.setLayoutParams(new ViewGroup.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
            tempAlbumsList = new ListView(this);
            tempAlbumsList.setId(android.R.id.list);
            mListAlbumsFrame.addView(tempAlbumsList);
        }
        mListAlbums = tempAlbumsList;
        mNoResultAlbumsView = mListAlbumsFrame.findViewById(R.id.noResultLayout);
        View loadingAlbums = mListAlbumsFrame.findViewById(R.id.loadingLayout);
        if (loadingAlbums != null) {
            loadingAlbums.setVisibility(View.GONE);
        }
        
        // Initialize Songs list view frame
        View songsRoot = inflater.inflate(R.layout.browse, null, false);
        ListView tempSongsList = songsRoot.findViewById(android.R.id.list);
        if (tempSongsList != null && tempSongsList.getParent() instanceof ViewGroup) {
            mListSongsFrame = (ViewGroup) tempSongsList.getParent();
            // Remove from SwipeRefreshLayout parent so we can add it to ViewPager
            ((ViewGroup) mListSongsFrame.getParent()).removeView(mListSongsFrame);
            // Ensure proper layout parameters for ViewPager
            mListSongsFrame.setLayoutParams(new ViewGroup.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        } else {
            mListSongsFrame = new FrameLayout(this);
            mListSongsFrame.setLayoutParams(new ViewGroup.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
            tempSongsList = new ListView(this);
            tempSongsList.setId(android.R.id.list);
            mListSongsFrame.addView(tempSongsList);
        }
        mListSongs = tempSongsList;
        mNoResultSongsView = mListSongsFrame.findViewById(R.id.noResultLayout);
        View loadingSongs = mListSongsFrame.findViewById(R.id.loadingLayout);
        if (loadingSongs != null) {
    ...
    private void update(final ListView listView, final List<? extends Item> resultList,
        ...
        try {
            ...
            // Hide loading view for this specific list (find it in the list's parent frame)
            if (listView.getParent() instanceof ViewGroup) {
                View loadingView = ((ViewGroup) listView.getParent()).findViewById(R.id.loadingLayout);
                if (loadingView != null) {
                    loadingView.setVisibility(View.GONE);
                }
            }
            // Also hide the root loading view if it exists
    ...
    class SearchResultsPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(final ViewGroup container, final int position) {
            ...
            // Remove from any existing parent before adding to container
            if (v.getParent() != null) {
